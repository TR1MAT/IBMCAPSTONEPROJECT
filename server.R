...
shinyServer(function(input, output){
  ...
  if(input$City_dropdown=='All'){output$city_bike_map <- renderLeaflet({leaflet(cities_max_bike)
    %>%addTiles()%>%addCirclesMarkers(data = cities_max_bike,label = cities)})} 
  # If just one specific city was selected, then render a leaflet map with one marker
    else{output$city_bike_map <- renderLeaflet({leaflet(filteredData)
      %>%addTiles()%>%addCirclesMarkers(data = filteredData,label = input$City_dropdown)})
    }
